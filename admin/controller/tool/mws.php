   ...
   const ORDERNUMBER = "orderNumber";
	...
	public function index() {
		...
		if ($order_info) {
			...
			$this->load->model('extension/yamodule/return');
			...
			if (!$errors && $this->request->server['REQUEST_METHOD'] == 'POST' && $is_act_return && isset($this->request->post['return_sum'])){
				...
				if (!$return_error){
					...
					if (isset($respPayment['status'])){
						$this->model_extension_yamodule_return->addReturn(array(
							'amount'=>$amount,
							'cause'=>$cause,
			...
			$returns = $this->model_extension_yamodule_return->getSuccessReturns($inv);
			...
			$this->response->setOutput($this->load->view('extension/yamodule/mws_info'.$end_tpl, $data));
